...
def init_db():
    ...
    with Session(engine) as session:
        ...
        for idx, row in orders_df.iterrows():
            ...
            if len(parts) % 2 != 0:
                print(f"Warning: malformed article field '{article_field}' in row {idx}, skipping")
                continue
            ...
            try:
                ...
                print(f"Warning: invalid pickup point index '{pickup_index}' in row {idx}, skipping")
                continue

            # Проверка существования пункта выдачи
            if not session.query(PickupPoint).filter(PickupPoint.id == pickup_id).first():
            ...
            if pd.isna(order_date):
                print(f"Warning: invalid order date '{row['Дата заказа']}' in row {idx}, skipping order")
                continue
            ...
            if pd.notna(row['Дата доставки']):
                ...
                if pd.isna(issue_date):
                    print(f"Warning: invalid issue date '{row['Дата доставки']}' in row {idx}, will set to None")
            ...
            for article, qty in items:
                order = Order(
                    ...
                    issue_date=issue_date.date() if issue_date and not pd.isna(issue_date) else None,
                    user_id=None
                )
                session.add(order)
                orders_created += 1
        ...
        session.commit()
        print(f"Imported {orders_created} orders (from {len(orders_df)} original rows).")
